...
const addEventTitle = document.getElementById("js_add-event_title") as HTMLInputElement | null; // イベント追加のタイトル入力
const addEventDate = document.getElementById("js_add-event_date") as HTMLInputElement | null; // イベント追加の日付入力
const addEventTime = document.getElementsByClassName("js_add-event_time") as HTMLCollectionOf<HTMLSelectElement> | null; // イベント追加の時間入力 配列のこと
const addEventBtn = document.getElementById("js_add-event_btn"); // イベント追加の追加ボタン

let editEventId:number | null= null; // イベント編集対象のid情報
const editEventTitle = document.getElementById("js_edit-event_title")as HTMLInputElement | null; // イベント編集のタイトル入力
const editEventDate = document.getElementById("js_edit-event_date")as HTMLInputElement | null; // イベント編集の日付入力
const editEventTime = document.getElementsByClassName("js_edit-event_time")as HTMLCollectionOf<HTMLSelectElement> | null; // イベント編集の時間入力
...
addEventBtn.addEventListener("click", () => {
  ...
  if(addEventTitle && addEventDate && addEventTime){
...
editEventBtn.addEventListener("click", () => {
  if(editEventTitle && editEventDate && editEventTime){
...
const showEvents = () => {
  ...
  thisEvents.forEach(event => {
    ...
    badgeElm.addEventListener("click", () => {
      editEventId = event.id; // 編集対象のidをセット
      editEventTitle.value = event.title; // 編集対象のタイトルをセット
      editEventDate.value = `${event.firstDate.getFullYear()}-${String(event.firstDate.getMonth() + 1).padStart(2, '0')}-${String(event.firstDate.getDate()).padStart(2, '0')}`; // 編集対象の日付をセット
      editEventTime[0].value = `${String(event.firstDate.getHours()).padStart(2, '0')}:${String(event.firstDate.getMinutes()).padStart(2, '0')}`; // 編集対象の開始時間をセット
      editEventTime[1].value = `${String(event.endDate.getHours()).padStart(2, '0')}:${String(event.endDate.getMinutes()).padStart(2, '0')}`; // 編集対象の終了時間をセット
    });
  });
}
...
const addTimeOption = () => {
  ...
  // editEventTime[0],[1]にoptionを追加する
